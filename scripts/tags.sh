...
regex_c=(
	...
	'/\<TESTPCGFLAG(\([[:alnum:]_]*\).*/PageCgroup\1/'
	'/\<SETPCGFLAG(\([[:alnum:]_]*\).*/SetPageCgroup\1/'
	'/\<CLEARPCGFLAG(\([[:alnum:]_]*\).*/ClearPageCgroup\1/'
	'/\<TESTCLEARPCGFLAG(\([[:alnum:]_]*\).*/TestClearPageCgroup\1/'
...
exuberant()
{
	...
	-I __devinitconst,__initdata_memblock			\
